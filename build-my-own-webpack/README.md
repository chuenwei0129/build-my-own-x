...
**首先在 `build-my-own-webpack` 目录：**

```sh
npm link
```

**然后在 [`test-my-own-webpack`](../test-my-own-webpack/) 目录：**

```sh
npm link build-my-own-webpack
```

就可以在 `examples` 目录执行 `pack` 命令进行打包了。在 `examples` 下的 `dist` 目录中测试打包结果。

## 手写实现

### 原理

> [webpack 原理分析与思考](https://github.com/chuenwei0129/awesome-me/blob/master/engineering/pack-origin.md)
...
class Compiler {
  ...
  emitFile() {
    // 把代码写入 ejs 模版
    // 输出路径和文件
    const bundle = path.join(this.config.output.path, this.config.output.filename)
    // 读取模版内容
